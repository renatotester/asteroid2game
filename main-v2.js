...
/* --- Joystick analógico (touch + pointer) --- */
const stickEl = document.getElementById('stick');
const nubEl   = document.getElementById('nub');
const stick = { active:false, dx:0, dy:0, mag:0, cx:0, cy:0, radius:70 };

function resetNub(){ nubEl.style.left="50%"; nubEl.style.top="50%"; nubEl.style.transform="translate(-50%,-50%)"; }
resetNub();
...
if(stickEl){
  ...
  stickEl.addEventListener("touchstart", e=>{
    e.preventDefault();
    const t=e.touches[0]; stickStart(t.clientX,t.clientY);
  },{passive:false});
  stickEl.addEventListener("touchmove", e=>{
    e.preventDefault();
    const t=e.touches[0]; if(stick.active) updateStick(t.clientX,t.clientY);
  },{passive:false});
...
function togglePause(){
  ...
  ovMsg.textContent='PAUSA — Toque ⏯ ou pressione P para continuar';
