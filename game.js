...
(function(){
  ...
  async function goFS(){
    try{
      if(!document.fullscreenElement){
        const el=wrap||document.documentElement; if(el.requestFullscreen) await el.requestFullscreen(); else throw 0;
      } else { if(document.exitFullscreen) await document.exitFullscreen(); }
    }catch(e){ wrap.classList.toggle('max'); }
  ...
  class UFO{
    ...
    update(dt){
      ...
      const ax=(aimX/len) * (this.tier===2?40:25) * DPR;
      const ay=(aimY/len) * (this.tier===2?40:25) * DPR;
  ...
  class PowerUp{
    constructor(x,y,type){ this.x=x; this.y=y; this.vx=rand(-50,50)*DPR; this.vy=rand(-50,50)*DPR; this.r=10*DPR; this.type=type; this.life=10; }
  ...
  function handleStick(e){
    ...
    const ndx = (dx/len||0)*k, ndy=(dy/len||0)*k;
    touch.dx = ndx; touch.dy = ndy;
    touch.thrust = len > 14;
    setNub(stick.clientWidth/2 + ndx*max, stick.clientHeight/2 + ndy*max);
  }
  if(stick){
    stick.addEventListener('pointerdown', onStickStart);
    addEventListener('pointermove', handleStick);
    addEventListener('pointerup', onStickEnd);
    addEventListener('pointercancel', onStickEnd);
  }
  const btnFire = document.getElementById('btnFire');
  const btnHyper= document.getElementById('btnHyper');
  const btnPause= document.getElementById('btnPause');
  btnFire?.addEventListener('pointerdown',()=>{ touch.fire=true; });
  ...
  function render(){
    ...
    for(const e of enemyShots) e.draw();
    for(const u of ufos) u.draw();
