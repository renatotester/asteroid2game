/* ASTRO OPS PRO+ — Build GitHub Pages friendly (cache-busted, sem ranking) */
(function(){
  ...
  function fitCanvas(){
    ...
    const w=(canvas.getBoundingClientRect().width||canvas.parentElement?.clientWidth||innerWidth);
    const h=(canvas.getBoundingClientRect().height||availH);
  ...
  async function goFS(){ try{ if(!document.fullscreenElement){ await (wrap.requestFullscreen?.call(wrap)); } else { await (document.exitFullscreen?.call(document)); } }catch(e){ wrap.classList.toggle('max'); } setTimeout(fitCanvas, 60); }
  btnFS.addEventListener('click', goFS);
  addEventListener('keydown', e=>{ if(e.key.toLowerCase()==='f'){ e.preventDefault(); goFS(); }});
  ...
  const applySkin = v => { document.body.classList.remove('skin-classic','skin-neon','skin-sunset','skin-dark','skin-mono'); document.body.classList.add(`skin-${v}`); localStorage.setItem('astroops.skin', v); };
  ...
  function setCRT(val){ document.body.classList.toggle('crt', val>0); document.documentElement.style.setProperty('--crtAlpha', String(val)); localStorage.setItem('astroops.crt', val); }
  ...
  function beep({freq=440,type='square',dur=.08,vol=.2,slide=0,attack=.002,release=.08}={}){ if(muted||!audio) return;
    const t0=audio.currentTime, o=audio.createOscillator(), g=audio.createGain(); o.type=type; o.frequency.setValueAtTime(freq,t0);
    if(slide) o.frequency.exponentialRampToValueAtTime(Math.max(50,freq*slide), t0+dur);
    g.gain.setValueAtTime(0,t0); g.gain.linearRampToValueAtTime(vol,t0+attack); g.gain.exponentialRampToValueAtTime(.0001,t0+dur+release);
  ...
  class Ship{
    ...
    update(dt){
      ...
      if(this.thr){ this.vx+=Math.cos(this.a)*acc*dt; this.vy+=Math.sin(this.a)*acc*dt; if(Math.random()<7*dt) sThru();
      ...
      const cd = this.rapid>0? 0.09 : 0.18; if(fireKey && this.cool<=0) this.shoot(cd);
      if(keys.has('h') || touch.hyper){ keys.delete('h'); touch.hyper=false; this.hyper(); }
      if(this.shield>0) this.shield-=dt; if(this.rapid>0) this.rapid-=dt;
    }
    shoot(nextCD){ const sp=700*DPR;
    ...
    hyper(){ sPow();
      for(let i=0;i<50;i++){ const x=rand(0,W()), y=rand(0,H());
        if(rocks.every(r=>dist2({x,y},r)>(r.r+80*DPR)**2) && ufos.every(u=>dist2({x,y},u)>(60*DPR)**2)){ this.x=x; this.y=y; this.vx=this.vy=0; this.inv=1.2; return; } } }
    hit(){ if(this.inv>0 || this.shield>0){ this.shield=Math.max(0,this.shield-0.6); sPow(); return; }
      lives--; sBoom(); shake=12*DPR; for(let i=0;i<26;i++) parts.push(new Part(this.x,this.y,rand(-240,240)*DPR,rand(-240,240)*DPR,rand(.25,.6),'bm'));
      if(lives>=0) this.reset(); else gameOver(); }
    draw(){ ctx.save(); ctx.translate(this.x,this.y); ctx.rotate(this.a);
      const blink=this.inv>0 && Math.floor(this.inv*10)%2===0; ctx.globalAlpha=blink?0.35:1; ctx.lineWidth=2*DPR; ctx.strokeStyle='#b3f3ff';
  ...
  class Rock{
    constructor(x,y,s=3,hp=0){ this.x=x; this.y=y; const sp=rand(20,70)*(4-s)*DPR, ang=rand(0,Math.PI*2);
      this.vx=Math.cos(ang)*sp; this.vy=Math.sin(ang)*sp; this.rot=rand(-1,1); this.a=rand(0,Math.PI*2); this.s=s; this.r=(s===4?70:s===3?46:s===2?28:16)*DPR; this.hp=hp||(s===4?6:s===3?3:s===2?2:1);
      const n=rint(9,14); this.poly=Array.from({length:n},(_,i)=>{const t=i/n*Math.PI*2, R=this.r*rand(.78,1.12); return {x:Math.cos(t)*R,y:Math.sin(t)*R};}); }
    ...
    draw(){ ctx.save(); ctx.translate(this.x,this.y); ctx.rotate(this.a); ctx.lineWidth=2*DPR; ctx.strokeStyle='#9bd4ff';
      ...
      if(this.s>=3){ ctx.rotate(-this.a); ctx.translate(-this.r, -this.r-10*DPR); const w=this.r*2, t=Math.max(0,Math.min(1,this.hp/(this.s===4?6:this.s===3?3:2)));
        ctx.fillStyle='rgba(0,0,0,.4)'; ctx.fillRect(0,0,w,4*DPR); ctx.fillStyle='rgba(155,212,255,.9)'; ctx.fillRect(0,0,w*t,4*DPR); }
      ctx.restore(); }
  }
  ...
  class UFO{
    constructor(tier=1){ const side=rint(0,4); this.x=side%2===0?(side===0?-40*DPR:W()+40*DPR):rand(0,W()); this.y=side%2===1?(side===1?-40*DPR:H()+40*DPR):rand(0,H());
      this.vx=rand(-60,60)*DPR; this.vy=rand(-60,60)*DPR; this.r=14*DPR; this.cool=rand(.6,1.2); this.tier=tier; this.hp=tier===2?3:1; }
    update(dt){ const ax=(ship.x-this.x)/(Math.hypot(ship.x-this.x,ship.y-this.y)||1)*(this.tier===2?40:25)*DPR;
      const ay=(ship.y-this.y)/(Math.hypot(ship.x-this.x,ship.y-this.y)||1)*(this.tier===2?40:25)*DPR;
      this.vx=Math.max(-180*DPR,Math.min(180*DPR,this.vx+ax*dt)); this.vy=Math.max(-180*DPR,Math.min(180*DPR,this.vy+ay*dt));
      this.x=wrap(this.x+this.vx*dt,W()); this.y=wrap(this.y+this.vy*dt,H()); this.cool-=dt; if(this.cool<=0){ this.shoot(); this.cool=this.tier===2?0.6:1.0; } }
    shoot(){ const dir=Math.atan2(ship.y-this.y,ship.x-this.x), sp=(this.tier===2?380:320)*DPR; enemyShots.push(new EnemyShot(this.x+Math.cos(dir)*this.r,this.y+Math.sin(dir)*this.r,Math.cos(dir)*sp,Math.sin(dir)*sp)); sUFOS(); }
  ...
  function spawnLevel(n=level){ rocks.length=0; ufos.length=0; enemyShots.length=0; powerups.length=0; const count=3+n; const boss=(n%5===0); if(boss) sBoss();
    for(let i=0;i<count;i++){ let x,y; do{ x=rand(0,W()); y=rand(0,H()); } while(ship && dist2({x,y},ship)<(200*DPR)**2); rocks.push(new Rock(x,y, boss && i===0 ? 4 : 3)); } ufoTimer=rand(6,10); }

  // --- Touch
  const touch={ thrust:false, fire:false, hyper:false, dx:0, dy:0 };
  const stick=document.getElementById('stick'); const nub=stick?.querySelector('.nub'); let stickActive=false,sx=0,sy=0;
  ...
  const btnFire=document.getElementById('btnFire'), btnHyper=document.getElementById('btnHyper'), btnPause=document.getElementById('btnPause');
  btnFire?.addEventListener('pointerdown',()=>{ touch.fire=true; }); btnFire?.addEventListener('pointerup',()=>{ touch.fire=false; });
  btnHyper?.addEventListener('click',()=>{ touch.hyper=true; });
  ...
  function update(dtRaw){
    ...
    for(let i=rocks.length-1;i>=0;i--){ const r=rocks[i];
      for(let j=bullets.length-1;j>=0;j--){ const b=bullets[j]; if(hitCC(b,r)){ b.life=0;
        if(r.hit()){ sBoom(); shake=Math.max(shake,(r.s>=4?18:12)*DPR); for(let k=0;k<(r.s>=3?18:12);k++) parts.push(new Part(b.x,b.y,rand(-250,250)*DPR,rand(-250,250)*DPR,rand(.18,.45),'bm'));
          rocks.splice(i,1); r.split(); if(Math.random()<0.15) spawnPower(b.x,b.y); score += r.s===4?300 : r.s===3?60 : r.s===2?90 : 140; elScore.textContent=score; addCombo(); } break; } } }
    for(let i=ufos.length-1;i>=0;i--){ const u=ufos[i];
      for(let j=bullets.length-1;j>=0;j--){ const b=bullets[j]; if(hitCC(b,u)){ b.life=0; if(u.hit()){ ufos.splice(i,1); shake=Math.max(shake,10*DPR); } break; } } }
    for(let i=enemyShots.length-1;i>=0;i--){ if(hitCC(enemyShots[i], ship)){ enemyShots.splice(i,1); ship.hit(); } }
    for(const r of rocks){ if(hitCC(ship,r)){ if(r.hit()){ sBoom(); rocks.splice(rocks.indexOf(r),1); r.split(); } ship.hit(); break; } }
    ...
    // fim de nível
    if(rocks.length===0 && enemyShots.length===0){ level++; elLevel.textContent=level; if(level%5===0) lives=Math.min(6,lives+1); spawnLevel(); }
  ...
  function render(){
    ...
    ship && ship.draw(); ctx.restore();

    // vidas
    ctx.save(); ctx.translate(12*DPR,14*DPR);
    for(let i=0;i<Math.max(0,lives);i++){ ctx.save(); ctx.translate(i*20*DPR,0); ctx.strokeStyle='var(--accent)'; ctx.lineWidth=2*DPR;
  ...
  function startGame(){ score=0; level=1; lives=3; elScore.textContent=0; elLevel.textContent=1; elCombo.textContent='1x';
    bullets=[]; rocks=[]; parts=[]; ufos=[]; enemyShots=[]; powerups=[]; combo=1; comboTimer=2.5; timeScale=1; shake=0; ship=new Ship(); spawnLevel();
    overlay.classList.add('hidden'); gameRunning=true; paused=false; canvas.focus({preventScroll:true}); }
  function gameOver(){ gameRunning=false; if(score>hiScore){ hiScore=score; localStorage.setItem('astroops.hi.pro+',hiScore); }
    elHi.textContent=hiScore; ovScore.textContent=score; ovHi.textContent=hiScore; ovLevel.textContent=level;
    ovTitle.textContent='GAME OVER'; ovSub.innerHTML='Pressione <b>R</b>, <b>Espaço</b>, <b>Enter</b> ou <b>Clique</b> para reiniciar'; overlay.classList.remove('hidden'); }
  ...
  // --- Touch object (precisa existir antes das chamadas)
  const touch = window.__touch ||= { thrust:false, fire:false, hyper:false, dx:0, dy:0 };
})();
